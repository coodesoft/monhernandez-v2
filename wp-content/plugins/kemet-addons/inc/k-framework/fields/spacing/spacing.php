  ...
  class KFW_Field_spacing extends KFW_Fields {
    ...
    public function render() {
      ...
      $args = wp_parse_args( $this->field, array(
        ...
        'top_placeholder'    => esc_html__( 'top', 'kemet-addons' ),
        'right_placeholder'  => esc_html__( 'right', 'kemet-addons' ),
        'bottom_placeholder' => esc_html__( 'bottom', 'kemet-addons' ),
        'left_placeholder'   => esc_html__( 'left', 'kemet-addons' ),
        'all_placeholder'    => esc_html__( 'all', 'kemet-addons' ),
        'top'                => true,
        'left'               => true,
        'bottom'             => true,
        'right'              => true,
      ...
      echo '<div class="kfw--inputs">';

      if( ! empty( $args['all'] ) ) {

        $placeholder = ( ! empty( $args['all_placeholder'] ) ) ? ' placeholder="'. $args['all_placeholder'] .'"' : '';

        echo '<div class="kfw--input">';
        echo ( ! empty( $args['all_icon'] ) ) ? '<span class="kfw--label kfw--icon">'. $args['all_icon'] .'</span>' : '';
        echo '<input type="number" name="'. $this->field_name('[all]') .'" value="'. $value['all'] .'"'. $placeholder .' class="kfw-input-number'. $is_unit .'" />';
        echo ( $unit ) ? '<span class="kfw--label kfw--unit">'. $args['units'][0] .'</span>' : '';
        echo '</div>';
        ...
        foreach( $properties as $property ) {
          ...
          echo '<div class="kfw--input">';
          echo ( ! empty( $args[$property.'_icon'] ) ) ? '<span class="kfw--label kfw--icon">'. $args[$property.'_icon'] .'</span>' : '';
          echo '<input type="number" name="'. $this->field_name('['. $property .']') .'" value="'. $value[$property] .'"'. $placeholder .' class="kfw-input-number'. $is_unit .'" />';
          echo ( $unit ) ? '<span class="kfw--label kfw--unit">'. $args['units'][0] .'</span>' : '';
          echo '</div>';
